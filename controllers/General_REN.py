...
class REN(nn.Module):
    ...
    def __init__(self, dim_in: int, dim_out: int, dim_internal: int,
                 l: int, bias=False, mode="l2stable", gamma=0.3, Q=None, R=None, S=None,
        ...
        self.x0 = nn.Parameter((torch.randn(1, dim_internal, device=device) * std))
        self.X = nn.Parameter((torch.randn(2 * dim_internal + l, 2 * dim_internal + l, device=device) * std))
        self.Y = nn.Parameter((torch.randn(dim_internal, dim_internal, device=device) * std))
        self.Z3 = nn.Parameter(torch.randn(abs(dim_out - dim_in), min(dim_out, dim_in), device=device) * std)
        self.X3 = nn.Parameter(torch.randn(min(dim_out, dim_in), min(dim_out, dim_in), device=device) * std)
        self.Y3 = nn.Parameter(torch.randn(min(dim_out, dim_in), min(dim_out, dim_in), device=device) * std)
        self.D12 = nn.Parameter(torch.randn(l, dim_in, device=device))
        #self.D21 = nn.Parameter((torch.randn(p, l, device=device) * std)) #set to zero later to enable
        # computable Networked RENs
        self.B2 = nn.Parameter((torch.randn(dim_internal, dim_in, device=device) * std))
        self.C2 = nn.Parameter((torch.randn(dim_out, dim_internal, device=device) * std))

        if bias:
            self.bx = nn.Parameter(torch.randn(dim_internal, device=device) * std)
            self.bv = nn.Parameter(torch.randn(l, device=device) * std)
            self.bu = nn.Parameter(torch.randn(dim_out, device=device) * std)
        else:
            self.bx = torch.zeros(dim_internal, device=device)
            self.bv = torch.zeros(l, device=device)
            self.bu = torch.zeros(dim_out, device=device)
        # # # # # # # # # Non-trainable parameters # # # # # # # # #
        # Auxiliary elements

        self.x = torch.zeros(1, dim_internal, device=device)
        self.epsilon = 0.001
        self.F = torch.zeros(dim_internal, dim_internal, device=device)
        self.B1 = torch.zeros(dim_internal, l, device=device)
        self.E = torch.zeros(dim_internal, dim_internal, device=device)
        self.Lambda = torch.ones(l, device=device)
        self.C1 = torch.zeros(l, dim_internal, device=device)
        self.D11 = torch.zeros(l, l, device=device)
        self.D22 = torch.zeros(dim_out, dim_in, device=device)
        self.P = torch.zeros(dim_internal, dim_internal, device=device)
        self.P_cal = torch.zeros(dim_internal, dim_internal, device=device)
        self.D21 = torch.zeros(dim_out, l, device=device)
