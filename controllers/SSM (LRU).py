...
class LRUP(nn.Module):
    ...
    def forward(self, input):
        ...
        if input.dim() == 3:
            ...
            A = torch.tile(Lambda, (1, input.shape[1]))
            # Apply Parallel Scan ang get state sequence (initial condition = self.state)
            states = pscan(A, inputBU, self.state) # dimensions: (State size,Seq_length, Batches)
            ...
            C_broadcasted = C_unsqueezed.expand(self.out_features, self.state_features, input.shape[1], input.shape[2])
            # Elementwise multiplication and then sum over V (the second dimension)
